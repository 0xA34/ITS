...
class EvaluationConfig:
    ...
    # Comparison settings
    compare_with_references: bool = True
    include_all_references: bool = False  # Include all or only relevant references
...
class BaselineEvaluator:
    ...
    def generate_report(
        ...
        Args:
            ...
            include_references: Whether to include reference methods comparison
        ...
        if include_references:
            # Add YOLO family for comparison
            yolo_refs = ReferenceDatabase.get_by_category(
                ReferenceDatabase.YOLO_METHODS[0].category
            )
            for ref in yolo_refs[:10]:  # Top 10 YOLO methods
                report.add_reference_method(
                    model_name=ref.name,
                    map_50=ref.map_50,
                    map_75=ref.map_75,
                    map_50_95=ref.map_50_95,
                    fps=ref.fps,
                    latency_ms=ref.latency_ms,
                    params_millions=ref.params_millions,
                    source=ref.source,
                )
...
def main():
    ...
    parser.add_argument(
        "--no-references",
        action="store_true",
        help="Don't include reference methods in comparison",
    )
    ...
    if args.benchmark_only:
        ...
        if results_file.exists():
            ...
            evaluator.generate_report(
                results, include_references=not args.no_references
            )
        else:
        ...
        # Run full evaluation
        results = evaluator.run_full_evaluation()
        evaluator.generate_report(results, include_references=not args.no_references)
