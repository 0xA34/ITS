...
Usage:
    ...
    report.load_from_benchmark_history()  # Load previous benchmark results
...
class ComparisonReport:
    ...
    Usage:
        ...
        report.load_from_benchmark_history()
    ...
    def add_from_benchmark_result(
        self,
        result: Any,
    ):
        """
        Add a model from a BenchmarkResult object.

        Args:
            result: BenchmarkResult from benchmark_history
        """
        metrics = ModelMetrics(
            ...
            params_millions=result.params_millions,
            flops_billions=result.flops_billions,
        ...
        self.models[result.model_name] = metrics
        logger.info(f"Added model from history: {result.model_name}")

    def load_from_benchmark_history(
        self,
        history_file: Optional[str] = None,
        exclude_models: Optional[list[str]] = None,
    ):
        """
        Load all previous benchmark results from history.

        Args:
            history_file: Path to history file (uses default if None)
            exclude_models: List of model names to exclude (e.g., current model)
        """
        ...
        for model_name, result in history.load_all().items():
            if model_name not in exclude:
                self.add_from_benchmark_result(result)

        logger.info(f"Loaded {len(history) - len(exclude)} models from benchmark history")
