...
class ValidationReport:
    ...
    def _calculate_validation_score(self) -> tuple[float, str]:
        ...
        if self.benchmark_comparison is not None:
            ...
            if bench_dict:
                # Regressions penalize score more than improvements boost it
                degraded = bench_dict.get("degraded", 0)
                improved = bench_dict.get("improved", 0)
                stable = bench_dict.get("stable", 0)
                total = bench_dict.get("total_metrics", 1)

                if total > 0:
                    score = 100 - (degraded / total * 50) + (improved / total * 10)
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if self.benchmark_comparison is not None:
            bench_dict = self._convert_to_dict(self.benchmark_comparison)
            if bench_dict:
                if bench_dict.get("overall_status") == "REGRESSION":
                    recommendations.append(
                        "Performance regression detected compared to previous benchmarks. "
                        "Check for changes in training data or hyperparameters."
                    )
                elif bench_dict.get("overall_status") == "NO_HISTORY":
                    recommendations.append(
                        "First benchmark run for this model. Results saved as baseline for future comparisons."
                    )
    ...
    def generate_markdown(self) -> str:
        ...
        if report.benchmark_comparison:
            ...
            lines.append(
                f"- **Improved:** {bench.get('improved', 0)} | **Stable:** {bench.get('stable', 0)} | **Degraded:** {bench.get('degraded', 0)}"
            )
            lines.append("")
            ...
            if "comparisons" in bench and bench["comparisons"]:
                ...
                lines.append("")
                lines.append("| Metric | Current | Previous | Diff % | Status |")
                lines.append("|--------|---------|----------|--------|--------|")

                for comp in bench["comparisons"]:
                    status_icon = {"IMPROVED": "↑", "STABLE": "=", "DEGRADED": "↓"}.get(
                        comp["status"], ""
                    )
                    lines.append(
                        f"| {comp['metric_name']} | {comp['current_value']:.4f} | "
                        f"{comp['previous_value']:.4f} | {comp['percentage_diff']:+.1f}% | "
...
"""
            ...
            html += f"<strong>Improved:</strong> {bench.get('improved', 0)} | <strong>Stable:</strong> {bench.get('stable', 0)} | <strong>Degraded:</strong> {bench.get('degraded', 0)}</p>"
...
"""
                ...
                <tr>
                    ...
                    <td>{comp["current_value"]:.4f}</td>
                    <td>{comp["previous_value"]:.4f}</td>
